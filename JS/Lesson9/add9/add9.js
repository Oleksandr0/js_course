...
//
//     if (typeof usersList[usersListElement] !== 'object'){
//
//     }
//
//     let divName = document.createElement('div');
//     divName.innerText = `${usersListElement.id}. ${usersListElement.name}`
//
...
//     for (const divAddress1 in divAddress) {
//         if (divAddress.children.length){
//
...
//
//             boxAddress.appendChild(divStreet)
//             divAddress.appendChild(boxAddress)
//         }
//     }
//
...
for (const user of usersList) {
    ...
    divUser.style.paddingBottom = '15px'
    for (const userElement in user) {
        if (typeof user[userElement] !== 'object') {
            let element = document.createElement('div');
            element.innerText = `${userElement}: ${user[userElement]}`;
            divUser.appendChild(element);
        } else  {
            let wrap = document.createElement('div');
            for (const itemkey in user[userElement]) {
                if (typeof user[userElement][itemkey] !== 'object') {
                    let item = document.createElement('div');
                    item.innerText = `${itemkey}: ${user[userElement][itemkey]}`;
                    wrap.appendChild(item);
                    divUser.appendChild(wrap);
                }else {
                    let wrapper = document.createElement('div');
                    for (const key in user[userElement][itemkey]) {
                        let part = document.createElement('div');
                        part.innerText = `${key}: ${user[userElement][itemkey][key]}`;
